...
function nav(){
  const items=[ ["dashboard",svgIcon("dashboard"),L("dashboard")], ["inventory",svgIcon("inventory"),L("inventory")], ["history",svgIcon("history"),L("history")] ];
...
function categoryChips(prefix=""){
  ...
  const boxes = [filterBox({active:!selected?.length, attrs:`${attr}="all"`, title:L('all')})]
    .concat(cats.map(c=>{
      ...
      return filterBox({active:selected?.includes(c), attrs:`${attr}="${esc(c)}"`, icon:catIcon(c), title:trCat(c), count});
    }));
...
function inventoryView(){
  ...
  return `<section class="inventory-screen ${inventoryContextClass()}">
    ...
    <div class="toolbar"><div class="search-row"><span class="search-icon">${svgIcon("search")}</span><input id="search" class="input" value="${esc(state.search)}" placeholder="${L("search")}"><button type="button" class="btn filter-tools" aria-label="Filters">${svgIcon("sliders")}</button></div></div>
